#!/usr/bin/env python3
"""
One-off diagnostic (not part of Phase 9's scoped deliverables): investigates why Live run #6's
cycle 11 (docs/PHASE9_FORWARD_TEST_CHECKPOINT.md) tripped MANAGE_ONLY posture with
unknown_real=(171922069,) -- a real MT5-side ticket with zero matching local StateStore record.
Same incident SHAPE as run #4's ticket 171909600: a runner MARKET position (171920077, opened
cycle 1) is locally recorded but was NOT in reconcile()'s `matched` set for cycle 11, suggesting
it already closed (SL/TP) by the time cycle 11 ran -- 171922069 is the suspected auto-generated
closing order/deal for it.
...
mt5_adapter/mcp_order_executor.py's real reconcile() call does.
"""
...
TICKETS = {171920077, 171922069}
FROM_DATE = "2026-08-09"
TO_DATE = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%d")

_logger = get_logger("mt5_mcp_trading.scripts.investigate_ticket_171922069")
    ...
    for t in sorted(TICKETS):
        ...
        if rec is None:
            ...
            print(f"  ticket={t}: status={rec.status} strategy={rec.strategy} magic={rec.magic} "
                  f"requested_sl={rec.requested_sl} requested_tp={rec.requested_tp} "
