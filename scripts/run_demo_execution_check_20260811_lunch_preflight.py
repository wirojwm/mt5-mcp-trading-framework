#!/usr/bin/env python3
"""
One-off diagnostic (not part of Phase 9's scoped deliverables): fresh read-only close-out check
before stopping the 2026-08-11 morning Step 7 session for lunch. Confirms real live MT5 state
(positions/pending orders) and reports which local StateStore records are still OPEN/
OPEN_UNPROTECTED so a clean before/after picture exists at the exact point the session paused.
...
async def main() -> None:
    ...
    print(f"\n=== Of {len(stale_open)} locally-OPEN records, {still_live_count} are confirmed "
          f"still live; {len(stale_open) - still_live_count} are stale bookkeeping (unrelated "
          f"to today's runs, pre-existing debt) ===")
    print("=====================================================\n")
