#!/usr/bin/env python3
"""
Reconciles ticket 171651878 (grid BUY_LIMIT, magic=71101, Step 27's live loop run), found absent
from both live positions and live pending orders on a follow-up read-only check -- most likely
filled then closed via its own broker-side SL/TP, the same pattern observed repeatedly throughout
this project (Steps 9, 15, 17, 20, 26). Local StateStore still shows it as OPEN (stale, per Step
24's already-traced-harmless finding), since no close_position()/cancel() call was ever made for
it by this project.

Ticket 171651879 (the other grid order from the same run) is deliberately NOT touched here -- it
is still a live pending order, not stale, and the user's instruction was to reconcile only the
stale record.
...
- Re-verifies TICKET is absent from both live positions and live pending orders immediately
  before acting (not trusting the earlier report -- state can move between a report and an
  action); aborts with no action if it turns out to still be live.
- Exactly one direct StateStore.record_closed() call, no MCP call at all.
...
SYMBOL = "BTCUSD"
TICKET = 171651878  # grid BUY_LIMIT, confirmed absent from live state on a prior read-only check
...
async def main() -> None:
    ...
    print(f"=== Reconciling {TICKET} via StateStore.record_closed() only -- no MCP call ===")
    ...
    async with demo_execution_session(
        ...
        passed = state_store.lookup(TICKET) is not None and state_store.lookup(TICKET).status == "CLOSED"
