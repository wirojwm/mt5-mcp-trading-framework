#!/usr/bin/env python3
"""
Reconciles 4 tickets from the fifth live loop run (2026-08-04), found absent from both live
positions and live pending orders on a follow-up read-only check -- most likely filled then
closed via their own broker-side SL/TP, the same pattern observed repeatedly throughout this
project (Steps 9, 15, 17, 20, 26, 27). Local StateStore still shows all 4 as OPEN (stale, per
Step 24's already-traced-harmless finding), since no close_position()/cancel() call was ever made
for any of them by this project.

TICKETS = (171652731, 171652799, 171652801, 171652846)  -- 2 grid SELL_LIMIT, 2 runner BUY MARKET

The remaining 7 live tickets from this run (3 positions, 4 pending grid orders) are deliberately
NOT touched here -- they are still genuinely live, not stale.

One-off, narrowly-scoped script: acts on ONLY these four hardcoded tickets, nothing else, ever.
...
- Re-verifies live state for all 4 tickets immediately before acting (not trusting the earlier
  ...
  ANY ticket, if even one turns out to still be live.
...
SYMBOL = "BTCUSD"
TICKETS = (171652731, 171652799, 171652801, 171652846)
