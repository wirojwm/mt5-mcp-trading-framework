...
(2026-08-10) local StateStore records left "OPEN"/"OPEN_UNPROTECTED" by Step 7 live runs #4/#5
against real current MT5 state, after the user manually closed/cancelled all remaining demo
positions and pending orders before the lunch break. The read-only pre-flight
(run_demo_execution_check_20260810_afternoon_preflight.py) already confirmed the account is flat
(0 positions, 0 orders) and that every 2026-08-10 OPEN/OPEN_UNPROTECTED record is genuinely no
longer live -- this just applies the same closed-vs-cancelled classification used on 2026-08-07.

For each such ticket:
- Still a real live position or pending order -> left untouched (would be a bug if any showed up
  here, since the pre-flight already found the account flat).
- Not live, but has a matching Deal (filled/closed at some point, incl. manual closes) ->
  record_closed(), closed_at derived from the real OUT-entry deal time, corrected for Deal.time's
  confirmed +UTC mislabel via infer_deal_time_offset().
- Not live, no matching Deal at all (a grid LIMIT order that never filled, incl. manually
  cancelled pendings) -> record_cancelled(), closed_at = now.
...
SYMBOL = "BTCUSD"
TODAY_PREFIX = "2026-08-10"

_logger = get_logger("mt5_mcp_trading.scripts.reconcile_20260810_afternoon_leftovers")
    ...
    async with demo_execution_session(
        ...
        for ticket, closed_at in closed:
            state_store.record_closed(
                ticket,
                reason="reconciled 2026-08-10 afternoon: filled deal confirmed, absent from live state after manual close/cancel before lunch break",
                closed_at=closed_at,
            )
        for ticket in cancelled:
            state_store.record_cancelled(
                ticket,
                reason="reconciled 2026-08-10 afternoon: no deal found, absent from live pending orders after manual close/cancel before lunch break",
                closed_at=now,
            )
