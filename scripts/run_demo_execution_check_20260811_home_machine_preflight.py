#!/usr/bin/env python3
"""
One-off diagnostic (not part of Phase 9's scoped deliverables): fresh read-only pre-flight for the
FIRST session on the home machine, continuing after work-machine run #10 and its end-of-day
close-out (docs/PHASE9_FORWARD_TEST_CHECKPOINT.md, 2026-08-11 entries) -- confirms real account
state and reports local StateStore records before anything on this machine assumes ownership of
any ticket it finds live. `var/order_state` is machine-local and gitignored, so this machine's
local StateStore has no memory of the work machine's trades by design; this script exists to show
that plainly rather than let it pass unnoticed.
...
async def main() -> None:
    ...
    async with demo_execution_session(
        ...
        deals = await McpDealHistoryReader(client).get_deals(
            from_date="2026-08-01", to_date="2026-08-12",
        )
    ...
    print(f"\n=== Local StateStore records still OPEN/OPEN_UNPROTECTED: {len(stale_open)} "
          f"(this machine's var/order_state is gitignored and was never synced from the work "
          f"machine -- an empty/small count here is expected, not evidence of a clean account) ===")
