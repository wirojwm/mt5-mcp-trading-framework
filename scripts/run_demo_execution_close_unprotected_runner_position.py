#!/usr/bin/env python3
"""
One-off recovery for Step 18's live-verification run: ticket 171647565 (SELL MARKET, magic=72101
locally, strategy="runner") was placed successfully (retcode 10009) but its mandatory SL/TP
attach was rejected (retcode 10016, "Invalid stops") -- the exact same live-confirmed
retcode-trust bug already documented for ticket 171617865 (Phase 6 Step 6): the tool's own
message text claimed success while the real retcode said otherwise. Per the established design,
this correctly left the position OPEN_UNPROTECTED with no automatic retry or close attempted --
recovery is a separate, explicitly-approved action, which is what this script is.
...
scripts/run_demo_execution_close_pipeline_open_items.py: re-verify live state immediately before
acting, abort if it doesn't match the premise, exactly one close_position() attempt, no retry,
verify absence afterward.
...
SYMBOL = "BTCUSD"
TICKET = 171647565  # OPEN_UNPROTECTED runner position, SL/TP attach rejected (retcode 10016)
