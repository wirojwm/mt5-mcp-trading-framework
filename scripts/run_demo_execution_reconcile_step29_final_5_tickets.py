#!/usr/bin/env python3
"""
Reconciles the final 5 tickets from Step 29's live loop run (2026-08-04): 171652732 (runner
position), 171652730, 171652797, 171652844, 171653004 (grid pending orders). A read-only
reconciliation check found all 5 already absent from both live positions and live pending
orders -- most likely filled/triggered via their own broker-side SL/TP, the same pattern
observed repeatedly throughout this project (Steps 9, 15, 17, 20, 26, 27, 29). Local StateStore
still shows all 5 as OPEN (stale, per Step 24's already-traced-harmless finding), since no
close_position()/cancel() call was ever made for any of them by this project.
...
- Re-verifies live state for all 5 tickets immediately before acting (not trusting the earlier
