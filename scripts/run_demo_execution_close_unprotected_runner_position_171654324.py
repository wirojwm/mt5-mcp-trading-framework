#!/usr/bin/env python3
"""
One-off recovery for Step 30's cycle 3: ticket 171654324 (SELL MARKET, magic=72101 locally,
strategy="runner") was placed successfully (retcode 10009) but its mandatory SL/TP attach was
rejected -- retcode 10016, "Invalid stops" -- the fifth live-confirmed instance of the same
retcode-trust bug already root-caused in Step 28 (tickets 171617865, 171647565, 171651880,
171653006): the tool's own message text claimed success while the real retcode said otherwise.
Per the established design, this correctly left the position OPEN_UNPROTECTED with no automatic
retry or close attempted, and correctly stopped the loop after cycle 3 (no error tolerance) --
...
script (Step 19, Step 27, Step 29): re-verify live state immediately before acting, abort if it
doesn't match the premise, exactly one close_position() attempt, no retry, verify absence
afterward.
...
SYMBOL = "BTCUSD"
TICKET = 171654324  # OPEN_UNPROTECTED runner position, SL/TP attach rejected (retcode 10016)
