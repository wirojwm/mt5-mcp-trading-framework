...
| 3 | Both strategies independently reach the 30-trade minimum sample (Phase 8 Step 1's own bar) on **live**, not backtested, data, with real expectancy/drawdown reported either way | **PARTIAL — updated 2026-08-13**, real read on the home/work machine (`run_demo_execution_live_performance_monitor.py`, now also reporting win_rate/profit_factor/slippage) against the real `StateStore`/`get_deals` history including run #10: grid 90 trades (MET) / −0.566 R expectancy / 51.332 R max drawdown / 36.7% win rate / 0.199 profit factor; runner 28 trades (NOT MET, 2 short of the 30-trade bar) / −0.958 R expectancy / 31.586 R max drawdown / 14.3% win rate / 0.182 profit factor. 2 local records skipped (no matching OUT-entry deal); 5 matched trades carried a magic outside {71101, 72101}, excluded from both totals | `docs/PHASE9_FORWARD_TEST_CHECKPOINT.md`, Step 7 run #10; live-performance-monitor output, 2026-08-13 |
| 4 | Backtest-vs-forward expectancy drift characterized for both strategies (not required positive — just honestly understood) | **MET for grid, PARTIAL for runner — updated 2026-08-13**: grid's real 90-trade sample (−0.566 R expectancy, 36.7% win rate, 0.199 profit factor) stays negative, directionally consistent with Phase 8's backtested finding, now on a materially larger real sample than 2026-08-07's 46; runner's real figures (−0.958 R expectancy, 31.586 R drawdown, 14.3% win rate, 0.182 profit factor) are also negative and directionally consistent with Phase 8, but the 28-trade sample is still 2 short of the 30-trade minimum-sample bar, so this remains PARTIAL rather than MET | same as row 3 |
...
by design); re-run successfully on this machine 2026-08-13. Rows 5–7 needed Step 7 to have actually happened; run #10
