...
| 3 | Both strategies independently reach the 30-trade minimum sample (Phase 8 Step 1's own bar) on **live**, not backtested, data, with real expectancy/drawdown reported either way | **MET for both — updated 2026-08-13 (afternoon)**: after the same-day StateStore backlog reconciliation (Stage A item 4) retroactively made 115 previously-stale `OPEN` records joinable, a fresh re-run of `run_demo_execution_live_performance_monitor.py` found grid **169 trades** (−0.683 R expectancy / 115.505 R max drawdown / 32.0% win rate / 0.154 profit factor) and runner **64 trades** (−0.607 R expectancy / 41.561 R max drawdown / 20.3% win rate / 0.361 profit factor). Both comfortably clear the 30-trade bar. The morning's read (grid 90/runner 28) is now known to have been an incomplete sample, captured before that reconciliation ran — superseded, not a separate data point | `docs/PHASE9_FORWARD_TEST_CHECKPOINT.md`; live-performance-monitor output, 2026-08-13 afternoon |
| 4 | Backtest-vs-forward expectancy drift characterized for both strategies (not required positive — just honestly understood) | **MET for both — updated 2026-08-13 (afternoon)**: grid's real 169-trade sample (−0.683 R expectancy, 32.0% win rate, 0.154 profit factor) stays negative, directionally consistent with Phase 8's backtested finding (−0.311 R held-out), on the largest real sample to date. Runner's real 64-trade sample (−0.607 R expectancy, 41.561 R drawdown, 20.3% win rate, 0.361 profit factor) is also negative; its divergence from Phase 8's held-out backtest figure (−0.100 R) is under active investigation as a Phase 8 continuation effort (see `docs/PHASE9_FORWARD_TEST_CHECKPOINT.md`) | same as row 3 |
...
refresh rows 3–4's real numbers at any time without needing a new sustained run — only from the
machine holding the real `var/order_state` records the script reads from (machine-local, gitignored
by design); re-run successfully on this machine 2026-08-13, twice in one day (morning: before the
StateStore backlog reconciliation, incomplete; afternoon: after, complete — see row 3/4 notes). A
stale-`OPEN`-record backlog can silently understate both trade counts until reconciled — re-run this
after any bulk reconciliation, not just on a fixed schedule. Rows 5–7 needed Step 7 to have actually happened; run #10
